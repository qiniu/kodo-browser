...
test(async t => {
  ...
  await browser.waitForExist('[uib-tooltip="上一级"]')
  ...
  await browser.waitForExist('div.item-block-name='+BUCKET)
  ...
  console.log('创建目录')
  await browser.click('button=创建目录')
  .waitForVisible('form[name=form1] #name1').setValue('form[name=form1] #name1', FOLDER)
  .click('button=确定')
  ...
  .waitForVisible('a.dropdown-item=删除').click('a.dropdown-item=删除')
  .waitForVisible('.btn-danger=确定').click('.btn-danger=确定')
