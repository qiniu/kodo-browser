...
UploadJob.prototype.start = function() {
  ...
  if (isDebug) {
    console.log("Strating upload ", self.from.path, " to ", self.to.key);
  }
...
UploadJob.prototype.stop = function() {
  ...
  if (isDebug) {
    console.log("Paused upload ", this.from.path);
  }
...
UploadJob.prototype.deleteFile = function() {
  ...
  self.oss.deleteObject({ Bucket: self.to.bucket, Key: self.to.key }, function(
    ...
    if (err) {
      console.error("Cleanup ", self.to.key, err);
    } else {
      console.log(
        "crc64 verifying failed, file [kodo://" +
          self.to.bucket +
          "/" +
          self.to.key +
          "] is deleted"
...
UploadJob.prototype.startUpload = function() {
  ...
  if (isDebug) {
    console.log("Chunked upload ", self.from.path);
  }
  ...
  util.prepareChunks(self.from.path, self.checkPoints, function(
    ...
    if (checkPoints.chunks.length == 1 && checkPoints.chunks[0].start == 0) {
      if (isDebug) {
        console.log("Single upload ", self.from.path);
      }

      self.uploadSingle();
    } else {
      if (isDebug) {
        console.log("Multipart upload ", self.from.path);
      }
...
UploadJob.prototype.uploadSingle = function() {
  ...
  fs.readFile(filePath, function(err, data) {
    ...
    var req = self.oss.putObject(params);
